...
sys.path.append('.')
from utils.load_dataset import HCPYASC
from utils.visualizer import plot_connectivity_matrix
from models.gan import Encoder, Decoder, Critics, trainer_critics, \
    trainer_generator_regressor
...
for dr in dropout_rate:
    for a in alphas:
        ...
        # ------ Setting ------- #
        # result
        result_dir = f'./data/tg-gan-sc/dr_{dr}_alpha_{a}'
        os.makedirs(result_dir, exist_ok=True)

        # ------ GAN ------ #
        # Initialize generator and critics
        n_regions = 379
        encoder = Encoder(n_features=64, n_regions=n_regions, dr_rate=dr)
        decoder = Decoder(n_features=64, n_regions=n_regions)
        critics = Critics(n_regions=n_regions, dr_rate=dr)
        regressor = BrainNetCNN(n_regions=n_regions, dr_rate=dr)
        ...
        # Optimizer
        alpha = 0.0001
        encoder_optimizer = optim.Adam(params=encoder.parameters(), lr=alpha,
                                       betas=(0.9, 0.999))
        decoder_optimizer = optim.Adam(params=decoder.parameters(), lr=alpha,
                                       betas=(0.9, 0.999))
        critics_optimizer = optim.Adam(params=critics.parameters(), lr=alpha,
                                       betas=(0.9, 0.999))
        regressor_optimizer = optim.Adam(params=regressor.parameters(),
                                         lr=alpha, betas=(0.9, 0.999))

        # Loss function
        loss_func = RMSELoss()

        # Early stopping
        paths = [f'{result_dir}/checkpoint_encoder',
                 f'{result_dir}/checkpoint_decoder',
                 f'{result_dir}/checkpoint_critics',
                 f'{result_dir}/checkpoint_regressor']
        earlystopping = EarlyStopping(start=500, patience=50, paths=paths,
                                      verbose=True)

        # Test matrix.shape
        test_matrix = [matrix for matrix, _ in valid_loader]
        test_matrix = torch.stack(test_matrix, dim=0).to(dtype=torch.float32,
                                                         device=device)
        test_matrix = test_matrix.reshape(
            (len(test_matrix), 1, n_regions, n_regions)
        )
        test_target = [target for _, target in valid_loader]
        test_target = torch.stack(test_target, dim=0).to(dtype=torch.float32,
                                                         device=device)
        ...
        for epoch in range(n_epochs):
            ...
            for i, data in tqdm(enumerate(train_loader),
                ...
                # Update critics
                epsilon = torch.randn(len(real), 1, 1, 1, device=device,
                                      requires_grad=True)
                ec, ew = trainer_critics(critics=critics,
                                         optimizer=critics_optimizer,
                                         real=real, fake=fake,
                ...
                if (i + 1) % n_critics == 0:
                    ...
                    et, ef, er = trainer_generator_regressor(critics=critics,
                                                             regressor=regressor,
            ...
            fake = fake.reshape(len(fake), n_regions, n_regions)

            fig = plt.figure(figsize=(12, 8))
            fig.subplots_adjust(wspace=0.3, hspace=0.3, top=0.9, bottom=0.1,
                                left=0.1, right=0.9)
            for i in range(2):
                ax = fig.add_subplot(3, 4, i + 1)
                ax = plot_connectivity_matrix(fig=fig, ax=ax, matrix=fake[i],
                                              cmap='gist_heat',
                                              vmin=0, vmax=15)
                ax.set_title(
                    f'Epoch: {epoch + 1}, Sample: {i + 1}, Score: {test_target[i]}',
                    fontsize=9)
            fig.savefig(
                f'{result_dir}/structural_connectivity_{epoch + 1}.pdf',
                transparent=True)
            plt.clf()
            plt.close(fig=fig)
            ...
            with open(f'{result_dir}/loss_epoch_{epoch + 1}.pkl',
                      mode='wb') as f:
                pickle.dump(obj=loss, file=f)

            if (epoch + 1) % 500 == 0:
                torch.save(encoder.state_dict(),
                           f'{result_dir}/checkpoint_encoder_epoch_{epoch + 1}.pth')
                torch.save(decoder.state_dict(),
                           f'{result_dir}/checkpoint_decoder_epoch_{epoch + 1}.pth')
                torch.save(critics.state_dict(),
                           f'{result_dir}/checkpoint_critics_epoch_{epoch + 1}.pth')
                torch.save(regressor.state_dict(),
                           f'{result_dir}/checkpoint_regressor_epoch_{epoch + 1}.pth')

            # Early stopping
            earlystopping(loss=r_val_error,
                          models=[encoder, decoder, critics, regressor])
