...
for dr in dropout:
    for a in alphas:
        ...
        fig.savefig('../fig/acquired-sc_mean_connectivity_matrix.pdf', transparent=True)

        fig = plt.figure()
        ax = fig.add_subplot()
        ax = plot_connectivity_matrix(fig=fig, ax=ax, matrix=fake_mean, cmap='gist_heat', vmin=0, vmax=2.0, labels=network_labels)
        fig.savefig('../fig/tg-gan-2-sc_mean_connectivity_matrix.pdf', transparent=True)
        ...
        # node_size = [(400 - 50)*(ns - min(node_strength))/(max(node_strength) - min(node_strength)) + 50 for ns in node_strength]
        #
        ...
        #
        # fig.savefig('../fig/acquired-sc_network_graph.pdf', transparent=True)
        #
        # # fake
        ...
        # nx.draw_networkx_nodes(G=G, pos=nx.circular_layout(G),
        #                        node_color=node_color, node_size=node_size,)
        # fig.savefig('../fig/tg-gan-2-sc_network_graph.pdf', transparent=True)
