...
# Visualize acquired and synthesized brain connectome in yeo's network order
...
for dr in dropout:
    for a in alphas:
        ...
        # fig = plt.figure()
        # ax = fig.add_subplot()
        # ax = plot_connectivity_matrix(fig=fig, ax=ax, matrix=real_mean, cmap='gist_heat', vmin=0, vmax=5, labels=network_labels)
        # fig.savefig('../fig/acquired-sc_network_matrix.pdf', transparent=True)
        #
        # fig = plt.figure()
        # ax = fig.add_subplot()
        # ax = plot_connectivity_matrix(fig=fig, ax=ax, matrix=fake_mean, cmap='gist_heat', vmin=0, vmax=5, labels=network_labels)
        # fig.savefig('../fig/tg-gan-2-sc_network_matrix.pdf', transparent=True)
        ...
        for ed in edge_density:
            ...
            fig = plt.figure()
            ax = fig.add_subplot()
            ax = plot_connectivity_matrix(fig=fig, ax=ax, matrix=real_bin, cmap='gray', vmin=0, vmax=1, labels=network_labels)
            fig.savefig(f'../fig/acquired-sc_network_matrix_edge_density_{ed}.pdf', transparent=True)

            fig = plt.figure()
            ax = fig.add_subplot()
            ax = plot_connectivity_matrix(fig=fig, ax=ax, matrix=fake_bin, cmap='gray', vmin=0, vmax=1, labels=network_labels)
            fig.savefig(f'../fig/tg-gan-2-sc_network_matrix_edge_density_{ed}.pdf', transparent=True)
            ...
            # fig.savefig('../fig/tg-gan-2-sc_connectome_glass-brain.pdf', transparent=True)
