...
X_real = load_structural_connectivity_hcp(participants_csv=hcp_fluid_intelligence_train_csv,
                                     data_dir='../../data_hcp/structural_connectivity_360')
X_real = sym_matrix_to_vec(X_real, discard_diagonal=True)
y_real = load_score(participants_csv=hcp_fluid_intelligence_train_csv, target='fluid intelligence')


# Fake
synthesized_connectivity_pkl = f'../../data_hcp/tg-gan-2-sc_hcp_fluid-intelligence/dr_{dropout}_alpha_{alpha}/tg-gan-2-sc_hcp_fluid-intelligence_n-splits_{n_splits}_matrices.pkl'
synthesized_target_pkl = f'../../data_hcp/tg-gan-2-sc_hcp_fluid-intelligence/dr_{dropout}_alpha_{alpha}/tg-gan-2-sc_hcp_fluid-intelligence_n-splits_{n_splits}_targets.pkl'
...
for i, (train_idx, valid_idx) in enumerate(rkf.split(X=X, y=y)):
    ...
    for j, param in enumerate(params):
        ...
        for k, (in_train_ix, in_valid_ix) in enumerate(cv_inner.split(X=X_train, y=y_train)):
            ...
            # Training
            model = Ridge(alpha=param, fit_intercept=True)
    ...
    best_model = Ridge(alpha=opt_param, fit_intercept=True)
...
# Save as pickle
result_dir = f'./data/tg-gan-2-sc_hcp_fluid-intelligence/ridge/n_splits_{n_splits}/outer_{n_outer_splits}_inner_{n_inner_splits}'
