...
fig_dir = f'./fig/tg-gan-2-sc_hcp_crystallised-intelligence/{algorithm}/n_splits_{n_splits}'
os.makedirs(fig_dir, exist_ok=True)

# ------- Load dataset ------ #
hcp_crystallised_intelligence_train_csv = '../../data_hcp/hcp-cognition/hcp_crystallised-intelligence/hcp_crystallised-intelligence_prediction-model_test.csv'
X = load_structural_connectivity_hcp(participants_csv=hcp_crystallised_intelligence_train_csv,
                                     data_dir='../../data_hcp/structural_connectivity_360')
X = sym_matrix_to_vec(X, discard_diagonal=True)
y = load_score(participants_csv=hcp_crystallised_intelligence_train_csv, target='crystallised intelligence')

print('X: ', X.shape)
print('y: ', y.shape)

# ------ Load result ------ #
outer_data_pkl = f'./data/tg-gan-2-sc_hcp_crystallised-intelligence/{algorithm}/n_splits_{n_splits}/outer_{n_outer_splits}_inner_{n_inner_splits}/outer.pkl'
...
score_pkl = f'./data/tg-gan-2-sc_hcp_crystallised-intelligence/{algorithm}/n_splits_{n_splits}/outer_{n_outer_splits}_inner_{n_inner_splits}/score.pkl'
