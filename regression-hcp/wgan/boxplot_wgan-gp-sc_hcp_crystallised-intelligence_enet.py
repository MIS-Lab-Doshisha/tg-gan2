...
wgan_test_r_removed = []
for i in n_splits:
    wgan_score_pkl = f'./data/wgan-gp-sc_hcp_crystallised-intelligence/{algorithm}/n_splits_{i}/outer_{n_outer_splits}_inner_{n_inner_splits}/score.pkl'
...
test_mse.to_csv('./wgan-gp-sc_hcp_crystallised-intelligence_enet_test_mse.csv')
test_r.to_csv('./wgan-gp-sc_hcp_crystallised-intelligence_enet_test_pearson-r.csv')

# Box plot
fig = plt.figure()
ax = fig.add_subplot()

color_pallete = ['#fef0d9', '#fdcc8a', '#fc8d59', '#e34a33', '#b30000', '#7C0A02']
...
fig.savefig('./fig/wgan-gp-sc_hcp_crystallised-intelligence_enet_test_rmse.pdf', transparent=True)
fig.savefig('./fig/wgan-gp-sc_hcp_crystallised-intelligence_enet_test_rmse.png', dpi=700)
...
fig.savefig('./fig/wgan-gp-sc_hcp_crystallised-intelligence_enet_test_r.pdf', transparent=True)
fig.savefig('./fig/wgan-gp-sc_hco_crystallised-intelligence_enet_test_r.png', dpi=700)

# T-test [baseline < wgan-gp-sc]
print('Pearson r')
p_val = []
for i, j in enumerate(n_splits):
    t, p = stats.ttest_ind(wgan_test_r_cleaned[i], baseline_test_r_cleaned, equal_var=False, alternative="greater")
    p_val.append(p)
    print('n_splits: ', j)
    print('t: ', t)
    print('p: ', p)
...
p_val = []
for i, j in enumerate(n_splits):
    #t, p = stats.ttest_ind(baseline_test_mse_cleaned, wgan_test_mse_cleaned[i], equal_var=False, alternative="less")
    t, p = stats.ttest_ind(wgan_test_mse_cleaned[i], baseline_test_mse_cleaned, equal_var=False, alternative="less")
